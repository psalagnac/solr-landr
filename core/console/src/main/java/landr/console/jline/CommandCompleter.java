...
import landr.parser.CommandStringLayout;
...
class CommandCompleter implements org.jline.reader.Completer {
  ...
  public void complete(LineReader reader, ParsedLine line, List<Candidate> candidates) {
    ...
    if (completions != null) {
      for (Completion c : completions) {
        Candidate candidate =
            new Candidate(c.getValue(), c.getDisplay(), null, null, null, null, c.isComplete());
  ...
  private List<Completion> completeStatementNames(ParsedLine line, CommandString string) {
    ...
    if (string != null) {
      ...
      // Special case if the cursor is before the typed input. We consider we have an
      // empty statement
  ...
  /**
   * Completion for the first token of a command line. We only resolve command names, ignoring other
   * parameters.
   */
  ...
  /**
   * Invoke custom completer if any. This is invoked only if we were able to resolve the command
   * name.
   */
  private List<Completion> completeCustomCommand(CommandDescriptor<?> command, ParsedLine line) {
    ...
    // The custom may require values of other argument (like if we want to resolve a shard/replica,
    // we also need the collection name).
