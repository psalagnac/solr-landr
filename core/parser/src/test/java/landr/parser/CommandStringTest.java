...
public class CommandStringTest {
  ...
  public void testCursorScopedParsing() {
    ...
    // first statement
    assertEquals(
        "first", Objects.requireNonNull(CommandString.parse(commands, 0)).getCommandName());
    assertEquals(
        "first", Objects.requireNonNull(CommandString.parse(commands, 3)).getCommandName());
    assertEquals(
        "first", Objects.requireNonNull(CommandString.parse(commands, 7)).getCommandName());

    // second statement
    assertEquals(
        "second", Objects.requireNonNull(CommandString.parse(commands, 11)).getCommandName());
