...
public class CommandTokenizerTest extends TokenizerTestBase {
  ...
  public void testSimpleTokens() {
    ...
    // happy path
    assertTokens(
        "echo foo bar", List.of("echo", "foo", "bar"), List.of(0, 5, 9), List.of(3, 7, 11));
  ...
  public void testPhrase() {
    ...
    // just a phrase
    assertTokens("\"the phrase\"", List.of("the phrase"), List.of(0), List.of(11));

    // single token phrase
    assertTokens("\"single\"", List.of("single"), List.of(0), List.of(7));

    // token and phrase
    assertTokens(
        "token \"and phrase\"", List.of("token", "and phrase"), List.of(0, 6), List.of(4, 17));

    // with whitespaces
    // we keep spaces in the phrase
    assertTokens("   \"the   phrase\"   ", List.of("the   phrase"), List.of(3), List.of(16));
  }
  ...
  public void testFilteringStatement() {
    assertTokens(
        "echo foo ; echo bar", 12, List.of("echo", "bar"), List.of(11, 16), List.of(14, 18));
  }

  private void assertTokens(
      String command, List<String> expected, List<Integer> starts, List<Integer> ends) {
    assertTokens(command, -1, expected, starts, ends);
  }

  private void assertTokens(
      String command, int cursor, List<String> expected, List<Integer> starts, List<Integer> ends) {
