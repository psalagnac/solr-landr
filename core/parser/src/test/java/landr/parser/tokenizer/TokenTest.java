...
public class TokenTest {
  ...
  public void testIndexOf() {
    ...
    Token base = builder.value("0123456789").positions(0, 9).build();
  ...
  public void testAppend() {
    ...
    assertEquals(
        new Token.Builder().value("hello world").positions(0, 10).build(), base.append(suffix));
    assertEquals(
        new Token.Builder().value("hello w").positions(0, 6).build(), base.append(suffix, 0, 0));
    assertEquals(
        new Token.Builder().value("hello orl").positions(0, 9).build(), base.append(suffix, 1, 3));
    assertEquals(
        new Token.Builder().value("hello d").positions(0, 10).build(), base.append(suffix, 4, 4));
  }
