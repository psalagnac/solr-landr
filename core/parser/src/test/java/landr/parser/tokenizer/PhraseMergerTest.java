...
public class PhraseMergerTest extends TokenizerTestBase {
  ...
  public void testTokens() {
    ...
    // Just two words !
    assertTokens(
        "foo bar", List.of("foo", "bar"), List.of(0, 4), List.of(2, 6), List.of(false, false));
  ...
  public void testMergePhrases() {
    ...
    // Simple phrase
    assertTokens("\"foo bar\"", List.of("foo bar"), List.of(0), List.of(8), List.of(true));

    // single token phrase
    assertTokens("\"single\"", List.of("single"), List.of(0), List.of(7), List.of(true));
    ...
    // With whitespaces.
    // We keep blank token before and after the phrase, but not the first space 'separator'
  ...
  public void testIndexes() {
    ...
    // Phrase
    assertTokenIndexes("\"foo bar\"", List.of("foo bar"), List.of(0), List.of(true));

    // Phrase followed by a token. There is nothing at position 1
    assertTokenIndexes(
        "\"foo bar\" token", List.of("foo bar", "token"), List.of(0, 2), List.of(true, false));
  }
  ...
  private void assertTokenIndexes(
      String command, List<String> expected, List<Integer> indexes, List<Boolean> phrases) {
    Iterator<Token> tokenizer = new Tokenizer(command);
    PhraseMerger merger = new PhraseMerger(tokenizer);
