...
public class CommandParser {
   ...
   *
   * <p>Encountered pragma are executed during parsing.
   */
  ...
  /**
   * Parses any statement. Can be a command or a pragma. Pragmas are immediately executed, while
   * commands are returned.
   *
   * <p>Note this method can return more than one command, if a single statement produces multiple
   * commands.
   *
  ...
  public List<Command> parse(ParserContext context, String line) throws CommandParseException {
    ...
    for (CommandString string : strings) {
      if (string.isPragma()) {
        ...
        // If the pragma also has an impact at execution time, introduce a command in the execution
        // chain
  ...
  private List<Command> parseCommand(ParserContext context, CommandString string)
  ...
  private CommandDescriptor<?> resolveCommand(ParserContext context, String name)
      throws CommandParseException {
  ...
  private PragmaDescriptor<?> resolvePragma(ParserContext context, String name)
      throws CommandParseException {
