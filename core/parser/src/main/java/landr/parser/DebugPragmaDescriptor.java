...
class DebugPragmaDescriptor extends PragmaDescriptor<DebugPragmaDescriptor.DebugPragma> {
    ...
    switch (name) {
      case VERBOSE_NAME:
        return new DebugAction(
            flag, CommandExecutionContext::setVerbose, ParserContext::setVerbose);
      case TRACE_NAME:
  ...
  /**
   * Internal action to change the debug flag. Can be either invoked in the command execution
   * context or the command parsing context.
   */
  private static class DebugAction {
    ...
    private DebugAction(
        boolean flag,
        BiConsumer<CommandExecutionContext, Boolean> commandAction,
        BiConsumer<ParserContext, Boolean> parserAction) {
      this.flag = flag;
    ...
    /**
     * Check whether this pragma has an impact at runtime in middle-of a script execution. When
     * true, we insert a {@link Command} to execute the action.
     */
