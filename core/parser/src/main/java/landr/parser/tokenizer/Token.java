 ...
 *
 * <p>Note that methods with bounds like {@link #subToken(int, int) subToken()} or {@link
 * #append(Token, int, int) append()} behaves differently from similar method in {@link String}. The
 * parameter for end position is always inclusive.
 */
public class Token {
  ...
  /**
   * Return the position of the token in its parent statement. If the command line has a single
   * statement, the returned value is the same as {@link #getIndex()}.
   */
  ...
  Token trim() {
    ...
    if (!Character.isWhitespace(buffer[bufferStart])
        && !Character.isWhitespace(buffer[bufferEnd])) {
      return this;
    }
  ...
  Token append(Token token) {
    // Unfortunately, we can't reuse the buffer. It may be used by another token after our end
    // position
    char[] newBuffer = new char[getLength() + token.getLength() + 1];
    System.arraycopy(buffer, bufferStart, newBuffer, 0, getLength());
    newBuffer[getLength()] = ' ';
    System.arraycopy(
        token.buffer, token.bufferStart, newBuffer, getLength() + 1, token.getLength());
  ...
  Token append(Token token, int start, int end) {
    ...
    newBuffer[getLength()] = ' ';
    System.arraycopy(
        token.buffer, token.bufferStart + start, newBuffer, getLength() + 1, end - start + 1);
  ...
  public boolean equals(Object obj) {
    ...
    boolean contentEquals =
        Arrays.equals(
            buffer, bufferStart, bufferEnd, other.buffer, other.bufferStart, other.bufferEnd);
  ...
  public String toString() {
    return String.format(
        "%s [start=%d, end=%d, index=%d, statementIndex=%d]",
