...
public abstract class SolrCommand extends Command {
  ...
  /**
   * Cast the solr client into a cloud aware client. An explicit exception is raised if the client
   * is not compatible.
   */
  protected CloudSolrClient toCloudClient(SolrClient client) throws CommandExecutionException {
    ...
    if (client instanceof CloudSolrClient) {
      ...
      throw new CommandExecutionException(
          new ClassCastException("Can't run command with non-cloud client"));
    }
  }

  /**
   * Utility method to send a synchronous command to the Solr cluster. Using this method instead of
   * directly calling SolrJ is preferred.
   */
  protected <R extends SolrResponse> R processRequest(
      CommandExecutionContext context, SolrRequest<R> request)
      throws SolrServerException, IOException, CommandExecutionException {
    return processRequest(context, request, null);
  }

  /**
   * Utility method to send a synchronous command to the Solr cluster, collection name is specified
   * to SolrJ. Using this method instead of directly calling SolrJ is preferred.
   */
  protected <R extends SolrResponse> R processRequest(
      CommandExecutionContext context, SolrRequest<R> request, String collection)
      throws SolrServerException, IOException, CommandExecutionException {
  ...
  /**
   * Recursive walkthrough all objects to display a JSon like output. This is mostly useful to print
   * a {@link NamedList} from a Solr response.
   */
  ...
  private void printNamedObject(
      CommandExecutionContext context, String name, Object object, int level) {
  ...
  private <K, V> void printEntries(
      CommandExecutionContext context, Iterable<Map.Entry<K, V>> entries, int level) {
    context.println("{");
